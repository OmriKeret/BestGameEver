...
public class EagleAILogic : MonoBehaviour, IEnemy
{
    ...
    public Vector3[] DiveToPlayer(Vector2 i_PlayerLocation)
    {
        Vector3 currentLocation = transform.position;
        Vector3 endLocation = new Vector3(0.8f * (currentLocation.x - i_PlayerLocation.x), currentLocation.y * 0.8f, 0);
        Vector3 controller1, controller2;
        if (currentLocation.x > endLocation.x)
        {
            controller1 = new Vector3(endLocation.x - 1.5f, i_PlayerLocation.y - 3.5f, 0);
        }
        else
        {
            controller1 = new Vector3(endLocation.x + 1.5f, i_PlayerLocation.y - 3.5f, 0);
